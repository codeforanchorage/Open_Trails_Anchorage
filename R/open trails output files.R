...
for(i in seq(dim(dat)[1])) {
 ...
 trail_segments$features[[i]] <- list(type = "Feature",
                                properties = list(id = as.character(dat$new_id[i]),
                                                  steward_id = dat$MANAGEMENT[i],
                                                  name = as.character(dat$TRAIL_NAME[i]), 
                                                  motor_vehicles = "no",
                                                  foot = "yes",
                                                  bicycle = "yes",
                                                  horse = "no",
...
fileConn<-file("output files/open trail standard/trail_segments.geojson")
...
named_trails <- data.frame(name = dat$TRAIL_NAME, 
                           segment_ids = dat$new_id, 
                           id = dat$ROUTEID,
                           description = rep("", dim(dat)[1]),
                           part_of = dat$SYSTEM_NAM)
# write named_trails.csv
write.csv(named_trails, file = "output files/open trail standard/named_trails.csv")
...
# write stewards.csv
write.csv(stewards, file = "output files/open trail standard/stewards.csv")

zip_dir <- c("output files/open trail standard")
